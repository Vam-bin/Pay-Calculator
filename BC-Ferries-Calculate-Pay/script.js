...
submitBtn.addEventListener("click", function (e) {
  if (e.key === "Enter") {
    ...
    if (shiftType.value === "FPTA") {
      calculatePayFPTA(startTime, endTime); // call the function to calculate the pay.
    }
  } else {
    e.preventDefault();
    const startTime = inputShiftStart.value; // get the start time values inputted by the user.
...
// Calculate the pay for shift -> FPTA
function calculatePayFPTA(startTime, endTime) {
  ...
  const regularPayRate_FPTA = 29.5; // Regular pay rate for FPTA.
  const payForHours = hoursWorked * regularPayRate_FPTA;
  const payForMinutes = 0.5 * regularPayRate_FPTA;
  ...
  if (startHour < 8) {
    ...
    calculatePayIfBefore_8AM(
      ...
      endMinutes,
      hoursWorked,
      regularPayRate_FPTA // regular-Pay-rate is passed by both the FPTA Shift and VB Shift with corresponding Pay Rates.
    );
  }
  ...
  // if the shift doesn't start before 8 am or if shift doesn't end after 5 pm.
    ...
    displayResult(totalPay);
  }
}

// Implement 15% more pay for hours before 8 AM.
function calculatePayIfBefore_8AM(
  ...
  displayResult(totalCumulativePay);

  setTimeout(function () {}, 1000);
}

// TODO - 1: Implement 15% more pay for hours after 5 pm.
...
// TODO - 2: Implement Vehicle Booth Pay.

// TODO - 3: Implement 15% more pay for VB.
